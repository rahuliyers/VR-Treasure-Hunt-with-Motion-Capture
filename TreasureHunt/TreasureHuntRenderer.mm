...
#import "BJNetworkManagment.hpp"


static BJNetworkManagement* _bjnm = nullptr;
...
//TODO: Read from UDP Server here
//TODO: Based on info that is come in, update state machine to determine if a punch is happening, and then make the cube disappear
- (void)update:(GVRHeadPose *)headPose {
    
    if(_bjnm != nullptr){
        _bjnm->update(1.0f/60.0f);
        
        NEUROMANCER_STATE state = _bjnm->getState();
        switch (state) {
            case NMS_CREATING_SERVER:{
                NSLog(@"Creating Server..");
                break;
            }case NMS_WAIT_ONE_SECOND_TILL_PACKETS_ARRIVE:{
                NSLog(@"Waiting for packets to arrive..");
                break;
            }case NMS_FIGURE_OUT_CORRECT_LOCAL_IP:{
                NSLog(@"Figuring out correct local IP..");
                break;
            }case NMS_SET_CURRENT_IP_AND_PORT:{
                NSLog(@"Setting Correct Local IP and Port..");
                if(_bjnm->isURLSet()){
                    std::string updateipportstring = _bjnm->getSetIPAndPortURL();
                    NSString *urlString = [NSString stringWithCString:updateipportstring.c_str()];
                    NSURL *url = [NSURL URLWithString:urlString];
                    NSData *data = [NSData dataWithContentsOfURL:url];
                    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                    //TODO: We aren't checking the response. If you get errors then try checking that the response is what you expect.
                    _bjnm->updateStateTo(NMS_RECEIVING_PACKETS);
                }

                break;
            }case NMS_SET_WAITING_FOR_RESPONSE_FOR_SET_IP_AND_PORT:{
                NSLog(@"Waiting for response to set ip..");
                break;
            }case NMS_RECEIVING_PACKETS:{
                auto messages = _bjnm->getMessages();
                if(messages.size() > 0){
                    //TODO: process each message
                    for (auto& message : messages){

                        auto& pitch = message[1];
